 ...
 * @module 1. Listado de Devoluciones
 * devolucion-lista.component.ts  
 */

 import { Component, ViewChild, OnInit, OnDestroy } from '@angular/core';
 import { ServiceinvoiceService } from '../devolucion.service';
 ...
 @Component({
   ...
   templateUrl: './devolucion-lista.component.html',
   styleUrls: ['./devolucion-lista.component.scss']
 })
  /** El nombre del modulo ListaDevolucionesComponent. */
 export class ListaDevolucionesComponent implements OnInit, OnDestroy {
 
   devolucionList:MatTableDataSource<any>;
   devolucionListOrig:any=[];
   ...
   constructor(private invoiceService: ServiceinvoiceService,
     ...
     private token: TokenService) {
      ...
      this.devolucionList = new MatTableDataSource();
      this.getUsuario(this.datSess[0],this.datSess[1]).subscribe(rsUs => {
        ...
        this.getDatos(this.datSess[1]).subscribe(resp => {
          ...
          this.Inicio('devoluciones','?idEmpresa='+this.datSess[1]);
        });
      });
   ...
   */
   Inicio(tabla:any, param:any){
    this.servicios.getUnParametro(tabla, param)
     .pipe(
         catchError(err => { this.devolucionList = new MatTableDataSource(); return throwError(err); })
     )
     .subscribe(
       (res:any) => {
        console.log(this.MonedaL);
        console.log(res);
         let devolucionListData= res;//.map( (t1:any) => ({ ...t1, ...this.arrsesion.find((t2:any) => t2.idUser === t1.idUser)  }) ) 
            devolucionListData.map((t1:any) =>{  
         ...
         this.devolucionList = new MatTableDataSource(devolucionListData);
         this.devolucionList.paginator = this.paginator!;
         this.devolucionList.sort = this.sort;
         this.devolucionListOrig=this.devolucionList.data; 
  ...
  */
   BuscarFiltro(json:any) {
    ...
    delete filtrado.titulo; 
     if(this.devolucionList.data.length != this.devolucionListOrig.length ){      
      this.devolucionList.data=this.devolucionListOrig;
      if(filtrado.fechaIni && filtrado.fechaFin){
        let filtroFecha=this.devolucionList.data.filter(function(item:any){
        ...
        const filteredData = funciones.buscarJsonEnJson(filtroFecha,filtrado);
         this.devolucionList.data=filteredData;        
       } else {
        delete filtrado.fechaIni;
        delete filtrado.fechaFin;
        const filteredData = funciones.buscarJsonEnJson(this.devolucionList.data,filtrado);
         this.devolucionList.data=filteredData;
       }
    } else {
      if(filtrado.fechaIni && filtrado.fechaFin){
   ...
   */
   isAllSelected() {
     ...
     const numRows = this.devolucionList.data.length;
   ...
   */
   masterToggle() {
       this.isAllSelected() ?
           ...
           this.devolucionList.data.forEach(row => this.selection.select(row));
   }
  /**
   * @function
   * @name checkboxLabel
   ...
   */
   exportar(){    
     this.selection.selected.length===0?this.generaExcel(this.devolucionList.data):this.generaExcel(this.selection.selected); 
   }
  /**
   * @function
   * @name generaExcel
   ...
   */
   generaExcel(arr:any){
     ...
     this.excelExporta.exportAsExcelFile(exportar, 'devoluciones_'+this.arrsesion[0].idUser); 
   }
 
   filter(filterValue: string) {
     this.devolucionList.filter = filterValue.trim().toLowerCase();
   }
 
   /////////////////////////////////////////////////////////////////////////////////////////////////////////// 
 
   deleteDevolucion(row: number) {/*
 
     if (confirm('Are you sure you want to delete this record ?')) {
       this.invoiceService.deleteDevolucion(row);
