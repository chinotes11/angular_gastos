...
import { ServiceinvoiceService } from '../conciliacion.service';
...
export class ListaConciliarComponent implements OnInit {
  ...
  transaccion: any;
  arrSelecto:any;
  displayedColumns: string[] = ['select','Empresa','IdColaborador','NombreColaborador','TipoTransaccion','IdTipoTransaccion','IdPago','FechaRegistroPago','MontoPago','Moneda','TipoCambio','ReferenciaPago','FormaPago','BancoEmisor','CuentaEmisora'];  @ViewChild(MatSort) sort: MatSort = Object.create(null);
  ...
  generaExcel(arr:any){
    ...
    let exportar:any=[];
      arr.map((ex:any) =>{
        exportar.push({
          Empresa:ex.Empresa,
          IdColaborador:ex.IdColaborador,
          NombreColaborador:ex.NombreColaborador,
          TipoTransaccion:ex.TipoTransaccion,
          IdTipoTransaccion:ex.IdTipoTransaccion,
          IdPago:ex.IdPago,
          FechaRegistroPago:moment(new Date(ex.FechaRegistroPago)).format("MM/DD/YYYY"),
          MontoPago:ex.MontoPago,
          Moneda:ex.Moneda,
          TipoCambio:ex.TipoCambio,
          ReferenciaPago:ex.ReferenciaPago,
          FormaPago:ex.FormaPago,
          BancoEmisor:ex.BancoEmisor,
          CuentaEmisora:ex.CuentaEmisora,  
        })        
      }); 
    this.excelExporta.exportAsExcelFile(exportar, 'conciliaciones_'+this.arrsesion[0].idUser); 
  }
