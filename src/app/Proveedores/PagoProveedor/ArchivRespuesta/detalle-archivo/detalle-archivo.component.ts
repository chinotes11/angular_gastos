...
import {  TesoreriaList} from '../archivo';
...
export class DetalleArchivoComponent implements OnInit {
  ...
  ListaColumns: string[] = ['IdColaborador', 'NombreColaborador','monto', 'Moneda','TipoCambio', 'FechaRegistroPago', 'FechaAplicacionPago', 'ReferenciaPago', 'Folio','ClaveRastreo', 'Estatus', 'FechaConciliacion'];
      ...
      .afterClosed().subscribe(res =>{
        ...
        if(res){
          let guardar={
            ...
            CuentaEmisora: this.tesoreriaListData.CuentaEmisora,
            FechaAprobacion: this.tesoreriaListData.createdAt,
  ...
  cerrarGastosZ() {    
    this.dialogService.openConfirmDialog('¿Esta seguro desea salir sin guardar?')
      .afterClosed().subscribe(res =>{
        if(res){
          this.router.navigate(['/proveedores/pagoproveedor/propuestaPago']);
        } 
    });
  }
