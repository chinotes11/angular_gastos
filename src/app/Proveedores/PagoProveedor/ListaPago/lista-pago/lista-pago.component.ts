...
export class ListaPagoComponent implements OnInit {
  ...
  TipoComprobante:any;
  arrSelecto:any;
  displayedColumns: string[] = ['select','Empresa','NombreColaborador','tipoTransaccion','IdTipoTransaccion','MontoTransaccion','FechaAprobacion','SaldoPendientePago','TipoCambio','IdPago','MontoPago','FechaAplicacionPago','FormaPago'];  @ViewChild(MatSort) sort: MatSort = Object.create(null);
    ...
    let exportar:any=[];
      arr.map((ex:any) =>{
        exportar.push({
         ...
         SaldoPendientePago:ex.SaldoPendientePago,
         TipoCambio:ex.TipoCambio,
         IdPago:ex.IdPago,
         MontoPago:ex.MontoPago,
         FechaAplicacionPago:moment(new Date(ex.FechaAplicacionPago)).format("MM/DD/YYYY"),
         FormaPago:ex.FormaPago,
        })        
      }); 
    this.excelExporta.exportAsExcelFile(exportar, 'ListaPagos_'+this.arrsesion[0].idUser); 
  }
